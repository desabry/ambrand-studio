...
export default function DashboardOverview() {
  ...
  useEffect(() => {
    async function load() {
      const [projectRes, msgRes, invRes, recentProjRes, recentInvRes] = await Promise.all([
  ...
  const statCards = [
    { label: "Total Projects", value: stats.projects, icon: FolderOpen, color: "text-brand", href: "/dashboard/projects" },
    { label: "Pending Messages", value: stats.messages, icon: MessageSquare, color: "text-amber-600", href: "#" },
    { label: "Revenue (Paid)", value: `$${stats.revenue.toLocaleString()}`, icon: Landmark, color: "text-emerald-600", href: "/dashboard/invoices" },
  ];
              ...
              <div key={inv.id} className="card p-5 space-y-3 hover:shadow-md transition-shadow">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`px-2 py-0.5 rounded-full text-xs font-medium capitalize border ${
                    inv.payment_status === "paid" ? "bg-emerald-50 text-emerald-700 border-emerald-100" :
