...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  console.log('[dashboard/layout] ===== LAYOUT RENDERING =====');
  ...
  console.log('[dashboard/layout] getUser result:', {
    userId: user?.id,
    hasError: !!userError,
  ...
  if (userError) {
    console.error('[dashboard/layout] getUser error detail:', userError);
  }
  ...
  console.log('[dashboard/layout] Profile query result:', {
    profile,
  ...
  if (profileError) {
    console.error('[dashboard/layout] Profile query error detail:', profileError);
  }

  const role = profile?.role ?? 'designer';
  console.log('[dashboard/layout] ✅ Resolved role:', role);
      ...
      <div className={cn(inter.className, "flex h-screen overflow-hidden bg-surface-50")}>
        <aside className="w-64 bg-white border-r border-surface-200 hidden md:flex flex-col shrink-0">
          ...
          <nav className="flex-1 p-4 space-y-1">
            ...
            {role === "owner" && (
              <>
                <a href="/dashboard/articles" className="flex items-center gap-3 px-3 py-2 text-surface-600 hover:text-brand hover:bg-brand-light rounded-lg transition-colors">
                  <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" x2="8" y1="13" y2="13"/><line x1="16" x2="8" y1="17" y2="17"/><polyline points="10 9 9 9 8 9"/></svg>
                  <span>Articles</span>
                </a>
                <a href="/dashboard/clients" className="flex items-center gap-3 px-3 py-2 text-surface-600 hover:text-brand hover:bg-brand-light rounded-lg transition-colors">
